...
def main():
    ...
    try:
        ...
        robot_id = input("Please enter SO100 robot ID (e.g.: right_follower_arm): ").strip()
        
        # If directly press enter, use default port
        if not port or not robot_id:
            print(f"Please input the port and ID")
            exit()
        else:
            print(f"Connecting to port: {port} with ID: {robot_id}")
