...
class FixedAxesJoyconRobotics(JoyconRobotics):
    ...
    def common_update(self):
        ...
        # 垂直摇杆：只控制X轴（前后）
        joycon_stick_v = self.joycon.get_stick_right_vertical() if self.joycon.is_right() else self.joycon.get_stick_right_vertical()
        ...
        for event_type, status in self.button.events():
            if (self.joycon.is_right() and event_type == 'plus' and status == 1) or (self.joycon.is_right() and event_type == 'minus' and status == 1):
                self.reset_button = 1
                self.reset_joycon()
            ...
            elif ((self.joycon.is_right() and event_type == 'zr') or (self.joycon.is_right() and event_type == 'zl')) and not self.change_down_to_gripper:
                self.gripper_toggle_button = status
            elif ((self.joycon.is_right() and event_type == 'stick_r_btn') or (self.joycon.is_right() and event_type == 'stick_l_btn')) and self.change_down_to_gripper:
                self.gripper_toggle_button = status
            else: 
                self.reset_button = 0
...
#     device="right",  # 改为右手控制器(已连接的Joy-Con)
