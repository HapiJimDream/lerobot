...
class SmoothBaseController:
    ...
    def update(self, pressed_keys, robot):
        ...
        # Debug output
        if DEBUG_BASE and any_key_pressed:
            print(f"[BASE] ACCEL: Speed={self.current_speed:.2f}, Action={base_action}")
        elif DEBUG_BASE and self.current_speed > 0.01:
            print(f"[BASE] DECEL: Speed={self.current_speed:.2f}, Action={base_action}")
        elif self.current_speed <= 0.01 and not self.has_reported_stopped:
            self.has_reported_stopped = True
...
def main():
    ...
    if not keyboard.is_connected:
        print(
            "[MAIN] Keyboard listener is not active. On macOS, grant Accessibility and Input Monitoring "
    ...
    try:
        ...
        while True:
            ...
            previous_pressed_keys = pressed_keys.copy()

            if robot.teleop_keys["quit"] in newly_pressed_keys:
                print("[MAIN] Quit key pressed, stopping teleoperation.")
                break
            if robot.teleop_keys["speed_up"] in newly_pressed_keys:
                robot.speed_index = min(robot.speed_index + 1, len(robot.speed_levels) - 1)
                speed_setting = robot.speed_levels[robot.speed_index]
                print(
                    f"[BASE] Speed level {robot.speed_index + 1}: "
                    f"Linear {speed_setting['linear']:.1f}m/s, Angular {speed_setting['angular']:.0f}°/s"
                )
